...
plot(log10(Size2), xlab=paste0("Barcode rank (", length(Size2)-elbow[1], " low quality cells)"), ylab = "log10 (lib size)", main = "Estimated Lib Sizes per Cell", col="darkblue", pch=16)
abline(v = elbow[1])
axis(1, at=elbow[1], labels=elbow[1])
plot(log10(Size2[1:elbow[1]]), xlab="Barcode rank", ylab = "log10 (lib size)", main = "Estimated Lib Sizes per high quality cell", col="darkblue", pch=16)
garbage <- dev.off()
