...
for barcode, name in P5fwdstr.items():
    ...
    barcodes3 = barcodeSet(barcode[0:6])
    barcodes4 = barcodeSet(barcode[0:5])
    # in case the index reads are trimmed
    for bc in barcodes:
        p5set[bc] = name
    for bc in barcodes2:
        p5set[bc] = name
...
for proj in project.values():
    f1 = io.BufferedWriter(open((proj + ".R1.fq"), 'ab'))
    f2 = io.BufferedWriter(open((proj + ".R2.fq"), 'ab'))
    files_r1[proj] = f1
    files_r2[proj] = f2
#print(files_r1)
...
#            print(i1_line)
 ...
 #       print(i1_line)    
        ...
        if (r2 in r2set):
            ...
            barcodeMatch += 1
        r3 = seqhead1[(index + 97):(index2)].decode()
...
#        p5 = seqhead1[(index + 106): -1].decode()
        if (p5 in p5set):
...
#                print(sampletype[p5set[p5]])
                if sampletype[p5set[p5]] == "ATAC" or sampletype[p5set[p5]] == "TAPS":
                    ...
                    needtrim = "T"
                elif sampletype[p5set[p5]] == "RNA" or sampletype[p5set[p5]] == "crop":
