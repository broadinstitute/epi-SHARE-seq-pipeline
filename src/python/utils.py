...
def check_putative_barcode(barcode_str, quality_str, barcode_exact_dict, barcode_mismatch_dict, barcode_merged_dict):
    """
    ...
    if corrected:
        ...
        if corrected:
            ...
            corrected = barcode_merged_dict.get(barcode_str[:8])  # check 1bp shift left
            quality = quality_str[:8]
            if corrected:
                correction_type = "L"
            else:  # check 1bp shift right; round 3 is shorter so add "N" for those
                if len(barcode_str) < 10:
                    corrected = barcode_merged_dict.get(barcode_str[2:]+"N")
                    quality = quality_str[2:]+"F"
                    if corrected:
                        correction_type = "R"
                else:
                    corrected = barcode_merged_dict.get(barcode_str[2:])
...
def create_barcode_dicts(barcode_list):
    ...
    barcode_merged_dict = dict()
    for barcode in barcode_list:
        barcode_exact_dict[barcode] = barcode  # exact match
        barcode_merged_dict[barcode] = barcode
        for i, base in enumerate(barcode):
            for x in 'ACGTN':
                if base != x:
                    ...
                    barcode_merged_dict[mismatch] = barcode
    return barcode_exact_dict, barcode_mismatch_dict, barcode_merged_dict
