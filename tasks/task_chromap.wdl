...
task atac_align_chromap {
    ...
    command <<<
        ...
        time chromap -x chromap_index/index \
                ...
                -t ~{cpus} \
                -1 ~{sep="," fastq_R1} \
                ~{if length(fastq_R2) > 0 then "-2" else ""} ~{sep="," fastq_R2} \
                ~{if length(fastq_barcode) > 0 then "-b" else ""} ~{sep="," fastq_barcode} \
        ...
        awk -v OFS="\t" -v maxinsert=~{max_insert_size} '$3-$2<=maxinsert' out.fragments.clipped.tsv | sort -k1,1 -k2,2n -k3,3n | bgzip -c > ~{fragments}.gz
