...
task qc_atac {
    ...
    command<<<
        ...
        #gzip -dc no-singleton.bed.gz | sort -k4,4 | gzip -c > no-singleton.sorted.bed.gz

        time python3 /usr/local/bin/snapatac2-tss-enrichment.py no-singleton.bed.gz gtf.gz ~{chrom_sizes} tss.extended.clipped.bed promoter.clipped.bed ~{fragment_cutoff} "~{prefix}.atac.qc.~{genome_name}.tss_enrichment_barcode_stats.tsv" "~{prefix}.atac.qc.~{genome_name}.tss_frags.png"
