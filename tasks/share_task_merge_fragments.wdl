...
task share_merge_fragments {
    ...
    command <<<
        ...
        # concatenate fragment files, re-sort, tabix index
        cat ~{sep=' ' fragments} | bgzip -dc | sort -k1,1 -k2,2n | bgzip -c > merged.sorted.frags.tsv.gz
        tabix -p bed merged.sorted.frags.tsv.gz > ~{output_file} 
    >>>
