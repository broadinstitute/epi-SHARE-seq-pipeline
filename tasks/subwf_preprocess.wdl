    ...
    task preprocess {
      ...
      output {
        ...
        File discarded_fastq_R1 = glob('out/*rna.R1.fq.gz')[0]
        File discarded_fastq_R2 = glob('out/*rna.R2.fq.gz')[0]
      }
    ...
    task gather_outputs {
        ...
        command <<<
            python <<CODE
                ...
                paths = ~{sep="," read_paths}
                paths_arr = paths.strip().split(",")
                
                idx = 0 
                
                fieldnames = ['entity:~{new_table_name}_Id', 'Name', 'Type', 'Primer']
                
                with open('results_load_table.tsv', 'w', newline='') as f_output:
                    csv_output = csv.DictWriter(f_output, fieldnames=fieldnames)
                    csv_output.writeheader()
                
                with open(~{yaml}) as f_input:
        ...
        runtime {
            docker: "polumechanos/update_terra_table"
    ...
    task entities_batch_upsert {
      ...
      command {
        ...
        python3 /software/batch_upsert_entities.py \
            -t "~{tsv_file}" \
            -p "~{terra_project}" \
            -w "~{workspace_name}"
      }
